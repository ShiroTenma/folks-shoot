...
import { Lock, Download, Image as ImageIcon } from 'lucide-react';

export default function PrivateAlbum() {
  const { id } = useParams(); // Mengambil Session ID dari URL
  ...
  // --- TAMPILAN 1: FORM PIN (KONDISI TERKUNCI) ---
  if (!photos) {
    return (
      <main className="min-h-screen bg-zinc-950 text-white flex flex-col items-center justify-center p-4">
        <div className="bg-zinc-900 p-8 rounded-2xl border border-zinc-800 w-full max-w-sm text-center">
          ...
          <p className="text-zinc-400 mb-6 text-sm">Masukkan PIN yang tertera di layar Photobooth.</p>
